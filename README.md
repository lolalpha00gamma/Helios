...
Klick = Maus-runter bei HALTEN, hoch beim Loslassen (wie ein echter Klick). Gesten-Test-HTML in der App.

## Neu in 1.6.111

Pinch-Klick/Zug nutzen dieselbe Schwelle wie die Distanz-Linie (roh ≥ 0,24). Datei-Overlay: Schließen-Kreis + Button in der Konsole.

## Neu in 1.6.110

Tastatur/Maus pausieren Helios 1,6 s. Klick auf den aktuellen Zeiger (AX-Press + CGEvent). Zug nur bei echtem Cursorweg, Mini-Zug wird Klick.

## Neu in 1.6.109

Aktionen: Pinch-Meter ignoriert Faust/offen. Rechtsklick blockt driveGrab nicht mehr. Ziehen trotz Maus-Pause. Pose-Floor nicht während Pinch.
...
Kein Ziehen im Idle. Schnabel Standard aus, offene Hand zählt nicht. Nur ein Overlay-Cursor (Steuerhand).

## Neu in 1.6.105

Cursor und Overlay wie 1.6.83: Palme → Bildschirm absolut, kein Highpass/Relativ, Overlay ohne Coast nach dem Sample.

## Neu in 1.6.104

Tap wieder über den Abstandszeiger, nicht pinchStartsGrab. Loslassen ohne Zug = Klick (ohne isClick/Freeze-Hitch).

## Neu in 1.6.103

Mac-Build: kein iOS-`inputPriority`/`isInterrupted`. Signiertes Latest.
...
- **hudCoastCapScaled palmWidth.** Fern kürzer, Nah länger. Overlay live.
...
Schnabel (Finger parallel zur Kamera) zieht Fenster — Schalter „Schnabel-Ziehen“. OK (Daumen+Zeigefinger, zwei Finger oben) öffnet Finder plus Ordner-Kreise.

## Neu in 1.6.82

Ruhende Hand ist keine Geste. Klatschen nur zwei offene Palmen. Overlay nicht nochmal nach vorn (Blackscreen). Lupe 2× am Cursor. Kneifen ab 0,24.
...
Der Abstandszeiger *ist* der Klick. Zu → auf feuert, ohne Reach/3D/Rechtsklick/Cooldown. Faust zählt nicht.
...
AXPosition-Fehler: Greifen hat die Fensterlage per Bedienungshilfen gelesen. Safari/Finder/Electron liefern das oft nicht. Tippen starb bei 8 fps in der Tentative-Phase.

- Ziehen = Maustaste halten (CGEvent), wie eine echte Maus. In jeder App.
- Ein geschlossener Continuity-Frame startet Halten. Kurze stillstehende Pinzette = Klick, auch ohne Closedness 0,50.
...
Kalibrierung hat Cursor, Klick und Wischen geschluckt und ließ sich nicht beenden.

- Abbruch: Button, Esc, 2× Klatschen, Faust. Overlay nimmt Klicks nur dann.
- Cursor bleibt während Kalibrierung und Pinch sichtbar (clamp, nicht hide).
- Offene Hand wischen = Schreibtisch (ctrl+Pfeil). Nach 4 Ecken automatisch Scharf.
