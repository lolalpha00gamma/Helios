...
1.6.98 Scale 1,6, Full/4 = 500 ms. Zweite Palme tot.

1. **visionRoiPeriodicFull every: 2.** 250 ms. Default 4 bleibt.
2. Tests + MARKETING_VERSION 1.6.99 (Build 132).

## Erweiterung (neu, 1.6.99)

1. **CameraBroker XPC + IOSurface** mit Aegis. P0.
2. **Adaptive ROI full every aus dt.** 8 Hz every 2 sitzt, 24 Hz every 6.
3. **Pinch-Release aus Kontakt-Fall** (dK/dt).
4. **Body-Pose @ 8 Hz jedes 8. Tick** statt immer skip.
5. **Wrist-Vel Pinch-Veto.**
6. **formatPromoted 720p24 → 1080p15.**
7. **Continuity 15-fps Probe.**
8. **VNTrackObjectRequest.**
9. **MediaPipe Hands.**
10. **Overlay CAMetalLayer.**
11. **LiDAR.**
12. **Per-Finger 4-Tip-Gate.**
13. **Hover-Dwell Click** aus analogClosed Hold.
14. **Stereo Continuity+Built-in zSep.**
15. **Gesture-Sequenzen Pinch+Flick.**
16. **Watch Digital Crown analog Zoom.**
17. **HeliosAegisKit.**
18. **DisplayLink Pause bei Freeze.**
19. **Cover-Lead Homographie Blend.**
20. **visionRoiScale lerp** statt Hart-Schwelle.

P0: CameraBroker. Kein neues *Need(dt). Predict nicht wieder an. Branch `bugfix` nicht mergen.

# Helios — Vorschlagsliste

Stand: **1.6.98**. Nur `main`. `bugfix` nicht mergen (1.6.15, 80 Versionen hinter main).

## In 1.6.98 erledigt

1.6.97 analog Mix. Continuity ROI Scale 3, HUD-Palme roh, Mutex Date() ohne Sample, SpaceMap nach Display tot.

1. **visionRoiScale(dt).** Continuity 1,6 / Built-in 3.
2. **hudLerp palmWidth.** Overlay clamp. Coast-Cap interpoliert.
3. **cameraMutexStampFresh Write.** Stamp+Lock nur frisches Sample.
4. **spaceMapSizeChanged.** Display → recenter.
5. Tests + MARKETING_VERSION 1.6.98 (Build 131).

## Erweiterung (neu, 1.6.98)
...
11. **Cover-Lead Homographie Blend.**
12. **Core Haptics** auf Pinch-Close.
13. **Gaze-Click** ARKit FaceAnchor + Pinch AND.
14. **visionRoiScale lerp** 1,6↔3 statt Hart 0,10 s.
15. **Mutex stamp+lock atomic.**
16. **SpaceMap Recenter ScreenParameters.**
17. **emptyHandsRecover(elapsed:)** Fold — recoverLive sitzt.
18. **sampleCursorYieldsToCoast** — Coast ohne DisplayLink tot.
19. **Vision joint-conf analog mix.**
20. **Per-Finger 4-Tip-Gate** (Kontakt ist ein Skalar).
21. **useCover Homographie.**
22. **Bezel-Hop Blend 200 ms.**
23. **Continuity sessionPreset-Watchdog** 3 s @ 8 fps.

P0: CameraBroker. Kein neues *Need(dt). Predict nicht wieder an. Branch `bugfix` nicht mergen.

# Helios — Vorschlagsliste (1.6.97)
...
1.6.89 Highpass/HUD. `cameraLocksExposure` / Center Stage tot in CameraSession — Continuity AE-Jagd + Crop.

1. **applyCaptureLocks.** Phone AE/WB locked, Mac continuous.
2. **applyCenterStage.** App-Modus + reassert aus. Session + Cover.
3. Tests + MARKETING_VERSION 1.6.90 (Build 123).

## Erweiterung (neu, 1.6.90)
...
29. **CGEvent tapHold.**
30. **pinch3DVeto Depth-Reach in driveGrab.**
31. **emptyHandsRecover(elapsed:)** Fold in Live.
32. **visionRoiEnabled** Continuity 8 Hz.
33. **Two-pinch zoom hysteresis.**
34. **Idle-Palm dwell** 400 ms nach Dropout.
35. **AE-Lock nach Wake reassert** (applyCenterStage force sitzt, Exposure nach Sleep?).
