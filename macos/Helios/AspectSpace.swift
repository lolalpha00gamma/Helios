...
/// Vision normalisiert x und y unabhängig auf [0, 1]. Abstände und
/// Geschwindigkeiten laufen in isotropen Einheiten: x' = x · (w/h), y' = y.
struct AspectSpace: Equatable {
    ...
    func iso(_ p: CGPoint) -> CGPoint {
        CGPoint(x: p.x * aspect, y: p.y)
    }

    func fromIso(_ p: CGPoint) -> CGPoint {
        CGPoint(x: p.x / max(0.01, aspect), y: p.y)
    }
...
enum JointGeom {
    ...
    static func extensionScore(angleRadians: CGFloat) -> CGFloat {
        let bent: CGFloat = 1.15
        let straight: CGFloat = .pi
        let t = (angleRadians - bent) / max(0.05, straight - bent)
