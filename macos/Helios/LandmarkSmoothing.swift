...
struct LandmarkSmoothing {
    ...
    mutating func reset() { previous.removeAll(); deriv.removeAll(); lastT = nil; jumps.removeAll() }
    mutating func apply(_ joints: [VNHumanHandPoseObservation.JointName: CGPoint], now: TimeInterval) -> [VNHumanHandPoseObservation.JointName: CGPoint] {
        defer { lastT = now }
        guard let lastT else { previous = joints; return joints }
        ...
        for (name, point) in joints {
            if let old = previous[name] { frameJumps.append(space.dist(old, point) / CGFloat(dt)) }
        }
    ...
    private func lerp(_ x: CGFloat, _ old: CGFloat, _ a: CGFloat) -> CGFloat { a * x + (1 - a) * old }
}
