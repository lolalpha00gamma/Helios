...
/// 12-Frame-Merkmalsfolge. Lädt optional `HeliosTemporal.mlmodelc` aus dem Bundle.
...
final class TemporalNet {
    ...
    init() {
        if let url = Bundle.main.url(forResource: "HeliosTemporal", withExtension: "mlmodelc")
            ?? Bundle.main.url(forResource: "HeliosTemporal", withExtension: "mlpackage")
            ?? Bundle.main.url(forResource: "HeliosTemporal", withExtension: "mlmodel")
        {
    ...
    private func inferHeuristic() -> HandEstimate {
        ...
        let dPinch = pinch - at(5, first)
        let dExt = zip(ext, (0..<5).map { at($0, first) }).map { $0 - $1 }
        let hold = dExt.map(abs).reduce(0, +) < 0.45 && abs(dPinch) < 0.18
    ...
    private func inferML(_ model: MLModel) -> HandEstimate? {
        ...
        guard let out = try? model.prediction(from: MLDictionaryFeatureProvider(dictionary: ["input": arr])),
              let scores = out.featureValue(for: "output")?.multiArrayValue
        else { return nil }
        var logits: [Double] = []
        ...
        return HandEstimate(
            source: .temporal,
            probabilities: probs,
            pinchClosedness: probs[.pinch] ?? 0,
            palm: .zero,
            palmVariance: 0.006,
            quality: Double(sm.max() ?? 0),
            available: true,
            palmWidth: 0.12
        )
    }
    ...
    static func features(
        ...
        f.append(Double(pinchRatio))
        f.append(Double(thumbUp))
        f.append(Double(palmVel))
        return f
    }
}
