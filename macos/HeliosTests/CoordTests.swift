...
enum CoordTests {
    ...
    static func main() {
        ...
        ok(CoordMath.chordLabel(shift: false, command: true, option: false) == "CMD", "Chord Peace = CMD")
        ok(CoordMath.chordLabel(shift: false, command: false, option: true) == "OPT", "Chord Point = OPT")
        ok(CoordMath.chordLabel(shift: true, command: false, option: false) == "SHIFT", "Chord Faust = SHIFT")
        ok(CoordMath.chordLabel(shift: false, command: false, option: false) == nil, "ohne zweite Hand kein Chord")
        ok(CoordMath.axPresses("AXCloseButton"), "Schließen AXPress")
        ok(CoordMath.axPresses("AXButton"), "Knopf AXPress")
        ok(!CoordMath.axPresses("AXSlider"), "Slider bleibt HID")
        ok(!CoordMath.axPresses(nil), "ohne Rolle kein AXPress")
        ok(CoordMath.peaceHoldDark(sinceScroll: 0.20), "Peace-HUD dunkel während Scroll")
        ok(!CoordMath.peaceHoldDark(sinceScroll: 0.80), "Peace-HUD hell nach Scroll")
        ok(CoordMath.isClapPulse(prevSpan: 3.2, prevT: 1.00, span: 1.05, now: 1.14), "Klatscher Pulse")
        ok(!CoordMath.isClapPulse(prevSpan: 3.0, prevT: 1.00, span: 1.2, now: 1.80), "zu langsam kein Klatscher")
        ok(CoordMath.isDoubleClap(first: 2.00, second: 2.45), "Doppelklatsch Gap")
        ok(!CoordMath.isDoubleClap(first: 2.00, second: 2.08), "zu eng kein Doppel")
