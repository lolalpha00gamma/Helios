...
enum GestureTests {
    ...
    static func main() {
        ...
        ok(GestureMath.pinchCloseNeed(dt: 0.125) == 2, "8 fps Close 2")
        ok(GestureMath.pinchCloseNeed(dt: 0.067) == 2, "15 fps Close 2")
        ...
        ok(abs(GestureMath.ghostRemain(emptyFor: 0) - 0.10) < 1e-6, "Ghost-Rest 0,10")
        ok(GestureMath.ghostRemain(emptyFor: 0.50) == 0, "Ghost-Rest nach Hold tot")
        ...
        ok(GestureMath.pinchTowardCamera(reach: 0.60, ratio: 0.28, prox: 0.30, tipsMissing: false, closeR: 0.33), "Z-Pinch 2D nah")
        ok(GestureMath.pinchTowardCamera(reach: 0.55, ratio: 0.50, prox: 0.28, tipsMissing: true, closeR: 0.33), "Z-Pinch Tips tot")
        ok(!GestureMath.pinchTowardCamera(reach: 0.95, ratio: 0.50, prox: 0.50, tipsMissing: false, closeR: 0.33), "seitlich weit kein Z")
        ...
        ok(!GestureMath.calibAborts(drift: 81), "Reichweite kein Pixel-Abbruch")
        ...
        ok(!GestureMath.armedIdle(faces: 0, lastFace: 1, lastInterior: 1, now: 2.3, hasLiveHand: true), "Live-Hand kein Gaze-Idle")
        ...
        ok(GestureMath.jointGain(count: 4) >= 0.84, "4 Joints Gain hält")
        ...
        ok(GestureMath.pinchCloseNeed(dt: 0.125, justOpened: false) == 2, "8 fps Close 2")
        ...
        ok(GestureMath.slotLateralityPrefers(slotCode: 2, liveCode: 1, haveMatch: true), "L/R sperrt Bind nicht")
        ...
        ok(!GestureMath.cameraMutexYieldsNow(holder: "aegis", owner: "aegis", wasYielded: true), "Yield löst wenn wir halten")
        ok(GestureMath.cameraMutexYieldReconfigure(yielded: true, isContinuity: true), "Yield → Built-in")
        ok(!GestureMath.cameraMutexYieldReconfigure(yielded: true, isContinuity: false), "Yield Built-in bleibt")
        ok(GestureMath.cameraMutexCacheFolder() == "HeliosAegis", "Mutex Caches-Ordner")
        ok(GestureMath.cameraMutexRelPath().hasSuffix("/helios.aegis.camera.lock"), "Mutex RelPath")
